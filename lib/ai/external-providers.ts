...
async function callFranceStudent(input: {
  ...
  const url = `${FS_API_BASE_URL}/chat/completions`;
  ...
  for (let keyIndex = 0; keyIndex < FS_KEYS.length; keyIndex++) {
    ...
    console.warn("🔁 Switching API key...");
  }
...
export type CometAction = "generate-image" | "edit-image";
...
export async function runCometImageModel(
  ...
  prompt: string,
  image?: string
): Promise<{ provider: string; imageUrl?: string }> {
  ...
  console.log("🖼️ Image request:", { action, model, prompt, image });
